...
/** run a sparql query at leipzig-data.de endpoint and request the result in
    json format, type can be 'select' or 'construct'
 */
$sp_events = array();
...
/** returns yyyy-mm-dd and inserts leading zeros for month and day if
    necessary
 */
...
function fetch_events($jahr,$monat) 
{
  ...
  //print_r($r);
  if (isset($r)) {	
    /* generate a data structure where for every day of the current month the
       list of events is stored */
	...
	/* evaluate dtstart and dtend of every event and "attach" it to the
	   days when it takes place */
	$date1 = date_create($s); 
	$date2 = ($e) ?  date_create($e) : date_create($s); 
	//assume duration of one day if dtend is not specified - NOTE
   ...
   <table cellspacing="0">
     <tr>
       <th class="termin" id="letzterMonat">
		<a href="'
    .get_param_url(array('syear'=>$link_alt_jahr,
			 'smonth'=>$link_alt_monat,
			 'sday'=>'0','eid'=>''))
    .'#buchungstabelle">&larr;</a>
       </th>
       <th colspan="5" id="monat">'.$namen[$monat].' '.$jahr.'</th>
       <th class="termin" id="naechsterMonat">
	  <a href="'.get_param_url(array('syear'=>$link_neu_jahr,
					 'smonth'=>$link_neu_monat,
					 'sday'=>'0','eid'=>''))
    .'#buchungstabelle">&rarr;</a>
       </th>
     </tr>
...
';
  ...
  for($i=$erster_tag;$i<=$letzter_tag;$i++){
	...
	$out.='
	  ...
	  .get_param_url(array('syear'=>$date['year'],
			       'smonth'=>$date['month'],
			       'sday'=>$date['day'],'eid'=>''))
	  .'#buchungstabelle">'
	  .$date['day'].'</a></td>';
  ...
  if ($eid) { 
    $out.='<td>'.printEventInfo($eid).'</td></tr></table>'; 
  }
  elseif ($tag) { 
    $out.='<td>'
      .printEventsPerDay(convert_date($jahr,$monat,$tag))
      .'</td></tr></table>'; 
  }
...
/** used to retrieve the current requested url and (re)set the given
    ...
    which have not been changed #### workaround for blogs without
    permalinks */
