 ...
 */
$triplify['db']=new PDO('mysql:host=localhost;dbname=name','user','passwd');
...
$triplify['queries']=array(
	'akteur'=>"SELECT ak.AID AS id, ak.name AS 'foaf:name', ak.email AS 'foaf:mbox', 
				ak.telefon AS 'foaf:phone', ak.url AS 'foaf:openid', 
				ak.funktion AS 'foaf:focus', ak.bild AS 'foaf:img',
				ak.beschreibung AS 'foaf:status', ak.oeffnungszeiten AS 'schem:opens', 					ak.modified AS 'xsd:dateTime', adr.plz AS 'schem:postalCode',
				CONCAT(adr.strasse,' ', adr.nr) AS 'schem:streetAddress',
         			adr.adresszusatz AS 'schem:addressLocality'
				FROM aae_data_akteur ak JOIN aae_data_adresse adr 
				ON ak.adresse=adr.ADID",
	'event'=>"SELECT ev.EID AS id, ev.name AS 'foaf:name', ev.kurzbeschreibung AS 'foaf:status',
				ev.bild AS 'foaf:img', ev.url AS 'foaf:openid', 
				ev.start_ts AS 'schem:opens', ev.ende_ts AS 'schem:closes', 
				ev.modified AS 'xsd:dateTime', akev.EID AS 'schem:organizer'
				FROM aae_data_event ev JOIN aae_data_akteur_hat_event akev 
				ON ev.EID=akev.EID"
);
...
$triplify['pingback']= array(
    'enabled' => false,          // Whether X-Pingback header should be exposed and XML-RPC is active.
    'write'   => false,          // Whether to write Pingbacks with the instance data.
);
