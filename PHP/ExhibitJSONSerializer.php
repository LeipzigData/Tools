...
class ExhibitJSONSerializer {
  ...
  function getSerializedIndex($resources){
    ...
    $exhibit = 
      array(
	    ...
	    'properties' => 
	    array(
		  'label'=> array('uri'=> 'http://www.w3.org/2000/01/rdf-schema#label'),
		  ),
	    'types' => 
	    array(
		  'item' => 
    ...
    foreach($resources as $uri => $properties) 
      //each uri has an array of properties, each property has an array of objects
      {
	...
	foreach($properties as $property => $objects)
	  {
		...
		default :
		  ...
		  break;
		case (empty($object['datatype']) 
		      AND $object['type']=='literal' 
		      AND !$this->isOtherType($property,$object) 
		      AND $this->hasHigherPriority('text',$exhibit['properties'][$qname]['valueType'] ) 
		      ):
		  $valuetype = 'text';
		  break;
		case (isset($resources[$object['value']]) 
		      AND ($object['type']=='uri' 
			   OR $object['type']=='iri')
		      ) :
		  $valuetype = 'item'; 
		  break;
		  //or the value is an URL in any case type should not 
		  // been determined as item in the past
							
		case (!isset($resources[$object['value']]) 
		      AND ($object['type']=='uri' 
			   OR $object['type']=='iri' 
			   OR $this->isValidURL($object['value'])) 
		      AND $this->hasHigherPriority('url',$exhibit['properties'][$qname]['valueType'])  
		      ): 
		  // last 3 conditions prevent overwriting valuetype 
		  // from item to url if to an uri no resource exists
		  $valuetype = 'url';
		  break;
		case (isset($object['datatype']) 
		      AND $object['datatype'] == self::XSD.'int'
		      ) :
		  $valuetype = 'number';
		  break;
		case ( (isset($object['datatype']) 
			AND $object['datatype'] == self::XSD.'date') 
		       OR $this->isDateProperty($property) 
		       ) : // last or condition to solve problems with ical
		  $valuetype = 'date';
		  break;
		case (isset($object['datatype']) 
		      AND $object['datatype'] == self::XSD.'boolean'
		      ) :
		  $valuetype = 'boolean';
		  break;						
		}
	    ...
	    if (!($property =='http://www.w3.org/1999/02/22-rdf-syntax-ns#type'))
	      // added condition otherwise [type] would be overwritten by object 
	      // instead of  converted exhibit json type name
	      {
  ...
  /* checks whether the predicate is an date predicate to set the value of the
     corresponding property to date solves the problem if date-objects don't
     have a date type */
    switch(true)
      {
      default : 
  ...
  /* Use in type-mapping switch to check if already a type has been detected
  for a property which has a higher prioritiy, thus e.g. a property with the
  type item won't be overwritten by the type url if for some object value no
  item exists */
    if (isset($lastType))
      {
	$result = true;
	switch($typeCase)
	  ...
	  case 'item' :	//item has highest priority 
	    break;
	  case 'text' : //text has lowest priority
	    if($lastType=='text')	
	      $result = true;
	    else $result = false;
	    break;	
